...
loại (classification) sử dụng mạng nơ-ron sâu (Deep Learning). Thay vì thiết kế một thống kê kiểm định thủ công dựa
...
Dựa trên kiến trúc MLP tổng quát đã trình bày ở Mục 2.3.1, phần này giải thích cách lựa chọn cấu hình
 cụ thể cho bài toán phát hiện điểm thay đổi, và đặc biệt là chứng minh rằng mạng nơ-ron có
  thể \textbf{bao hàm hoàn toàn} phương pháp CUSUM.

\subsubsection*{1. Tại sao cần đúng $2n-2$ nơ-ron?}

Một trong những kết quả lý thuyết quan trọng nhất của bài báo là chứng minh rằng thống kê CUSUM 
có thể được biểu diễn \textit{chính xác} bằng một mạng MLP 1 lớp ẩn với $2n-2$ nơ-ron ReLU.
...
Do đó, mỗi vị trí $k \in \{1, \ldots, n-1\}$ cần đúng \textbf{2 nơ-ron ReLU}: 
một cho $\max(w_k^\top X, 0)$ và một cho $\max(-w_k^\top X, 0)$. 
Với $n-1$ vị trí cần kiểm tra, tổng cộng cần $2(n-1) = 2n - 2$ nơ-ron ẩn.

\textbf{Ý nghĩa:} Kết quả này chứng minh rằng lớp hàm $\mathcal{H}_{1, (2n-2)}$ 
(mạng 1 lớp ẩn, $2n-2$ nơ-ron) đã đủ để \textit{chứa} bộ phân loại CUSUM. 
...
\subsubsection*{2. Tỉa mạng (Pruning): Từ $O(n)$ xuống $O(\log n)$ nơ-ron}

Mặc dù mạng $2n-2 = O(n)$ nơ-ron đảm bảo biểu diễn chính xác CUSUM, trong thực tế các nơ-ron 
...
Do đó, mạng đa lớp ($L$ lớp ẩn) với chỉ $m = 4\lfloor \log_2(n) \rfloor$ nơ-ron mỗi lớp đã đủ mạnh 
...
\subsubsection*{2. Tại sao dùng cấu trúc Thặng dư (Residual blocks)?}
...
Con số 21 khối thặng dư không phải được chọn ngẫu nhiên. 
Các tác giả đã tiến hành thử nghiệm thực nghiệm (exploring) với nhiều cấu trúc mạng khác nhau, 
thay đổi số lượng khối residual cũng như số lượng lớp fully connected trên các tập dữ liệu tự tạo. 
Kết quả cho thấy kiến trúc với \textbf{21 khối thặng dư} tiếp nối bởi các lớp fully connected 
...
\begin{itemize}
    \item \textbf{Backbone:} 21 khối Residual blocks với \texttt{base\_channels} = 32, \texttt{kernel\_size} = 8 cho mỗi lớp tích chập.
...
\subsubsection*{1. Cấu trúc tập dữ liệu mô phỏng}
Vì mạng nơ-ron cần lượng dữ liệu lớn để huấn luyện, nghiên cứu sử dụng dữ liệu mô phỏng. 
...
\begin{itemize}
    ...
    Như vậy, việc chọn $\mu_R$ theo công thức này đảm bảo dữ liệu mô phỏng nằm đúng 
...
\begin{itemize}
    ...
    Điều này giúp bộ phân loại bất biến với các phép biến đổi affine (cộng/nhân hằng số). 
    Tuy nhiên, đối với nhiễu đuôi nặng (kịch bản S3 - Cauchy), Min-Max sẽ thất bại do các giá trị cực đoan, 
    và nghiên cứu thay thế bằng phương pháp \textbf{chuẩn hóa mạnh} sử dụng trung bình cắt tỉa (trimmed mean) v
    à độ lệch chuẩn cắt tỉa (trimmed std) với tỷ lệ cắt 10\% mỗi phía.
    \item \textbf{Tăng cường dữ liệu (Data Augmentation):} Sử dụng kỹ thuật đảo ngược chuỗi $(x_n, x_{n-1}, \ldots, x_1)$. 
...
Mạng nơ-ron được huấn luyện để phân loại một đoạn dữ liệu kích thước cố định: \textit{có} hoặc \textit{không có} 
